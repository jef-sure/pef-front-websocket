...
my $server =
  ...
  ->send_arg( "", "", "", "", "" )
  ->run( 'PEF::Front::WebSocket::QueueServer::run', $cv );
...
$tt = AnyEvent->timer(
	...
	cb    => sub {
		is_deeply(
			$clients[0]{test}[0]->result,
			[ "test0", 1, { message => 'test message 1' } ],
			't 1.1'
		);
		is_deeply(
			$clients[1]{test}[0]->result,
			[ "test0", 1, { message => 'test message 1' } ],
			't 1.2'
		);
		is_deeply(
			$clients[2]{test}[0]->result,
			[ "test0", 1, { message => 'test message 1' } ],
			't 1.3'
		);
		is_deeply(
			$clients[0]{test}[1]->result,
			[ "test1", 2, { message => 'test message 2' } ],
			't 2.2'
		);
		is_deeply(
			$clients[0]{test}[2]->result,
			[ "test1", 2, { message => 'test message 2' } ],
			't 2.3'
		);
		is_deeply(
			$clients[1]{test}[1]->result,
			[ "test1", 2, { message => 'test message 2' } ],
			't 2.4'
		);
		is_deeply(
			$clients[0]{test}[3]->result,
			[ "test2", 3, { message => 'test message 3' } ],
			't 3.1'
		);
		is_deeply(
			$clients[1]{test}[3]->result,
			[ "test2", 3, { message => 'test message 3' } ],
			't 3.2'
		);
		is_deeply(
			$clients[2]{test}[3]->result,
			[ "test2", 3, { message => 'test message 3' } ],
			't 3.3'
		);
		is_deeply(
			$clients[1]{test}[2]->result,
			[ "test2", 3, { message => 'test message 3' } ],
			't 3.4'
		);
		is_deeply(
			$clients[2]{test}[2]->result,
			[ "test2", 3, { message => 'test message 3' } ],
			't 3.5'
		);
		$tt = AnyEvent->timer(
			...
			cb    => unblock_sub {
				$clients[1]{client}
				  ->unsubscribe( "test2", $clients[1]{test}[2] );
				$clients[1]{client}
				  ->publish( "test2", 4, { message => 'test message 4' } );
				$tt = AnyEvent->timer(
					after => 0.05,
					cb    => unblock_sub {
						is_deeply(
						...
						$clients[0]{client}
						  ->unregister_client( $clients[0]{test}[3] );
						$clients[0]{test}[3]->clear;
						$clients[0]{client}
						  ->subscribe( "test2", $clients[0]{test}[3], 3 );
						$tt = AnyEvent->timer(
							after => 0.05,
							cb    => sub {
								is_deeply(
